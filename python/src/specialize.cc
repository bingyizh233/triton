...
bool appendOptionalListMetadata(std::string &desc, PyObject *arg,
                                const char *name) {
  ...
  if (obj.ptr() == Py_None)
    return true;
  auto listObj = from_new_ref(PySequence_List(obj.ptr()));
  if (!listObj)
    return false;
  auto listStr = from_new_ref(PyObject_Str(listObj.ptr()));
  if (!listStr)
    return false;
  const char *listCStr = PyUnicode_AsUTF8(listStr.ptr());
  if (!listCStr)
    return false;
  desc += ",";
  desc += name;
