"""Warp-specialized multi-CTA forward convolution (implicit GEMM, NHWC).

This example mirrors ``03-matmul-multicta.py`` for convolution: a CLC
scheduler partition hands out output tiles, a producer partition issues TMA
...
def _validate_cga_layout(cga_layout):
    if not cga_layout:
        raise ValueError("multi-CTA convolution requires a non-empty M-split CGA layout")
    for basis in cga_layout:
        if len(basis) != 2 or basis[0] == 0 or basis[1] != 0:
            raise ValueError(f"Only M-split CGA layouts are supported, got {cga_layout!r}")
...
class MultiCTAConfig:
    ...
    def get_program(self, pid):
        ...
        return MultiCTAProgram(self, pid_m, pid_n)
...
@gluon.jit
def _multicta_load(p):
    """Producer: multicast TMA loads into the cluster-partitioned SMEM ring."""
    a_desc = p.a_desc
    b_desc = p.b_desc
    ...
    while scheduler.has_work:
        ...
        for k_iter in range(num_k_iter):
            ...
            tma.async_copy_global_to_shared(b_desc, [k_offset, off_n], bar, b_stage, multicast=True)
            state = state.next()
...
def _conv2d_im2col_multicta_ws_kernel(
    ...
    """Warp-specialized multi-CTA convolution with CLC scheduling."""
    ...
    mma_barrier_count: gl.constexpr = tcgen05_mma_barrier_count(
        [a_bufs.index(0), b_bufs.index(0)], multicast=True, two_ctas=TWO_CTAS,
    )
    ...
    # TMEM accumulator ring: cluster-split along M; each CTA physically
    # holds its [CTA_M, TILE_N] slice.
...
def _prepare_conv2d_inputs(input_tensor, weight_tensor, stride, padding, out=None):
    ...
    if input_tensor.dtype != TORCH_GEMM_DTYPE or weight_tensor.dtype != TORCH_GEMM_DTYPE:
        raise ValueError(
            f"conv2d_im2col_multicta_ws expects bf16 input/weight, got "
...
def conv2d_im2col_multicta_ws(
    ...
    *,
    ...
    """Warp-specialized multi-CTA forward convolution (MultiCTA).
    ...
    Default config uses a 4-CTA M-split CGA layout with TMA multicast:
    ...
    if M_GEMM % tile_m != 0:
        raise NotImplementedError(
            f"MultiCTA requires logical M to be a multiple of {tile_m}; got {M_GEMM}."
        )
...
@pytest.mark.parametrize("N,Ci,H,W,Co,R,S,stride,padding,block_size_n,epilogue_block_n,stages,cga_layout", [
    (128, 384, 64, 64, 384, 3, 3, 1, 1, 128, 128, 6, ((1, 0),)),
    (128, 384, 64, 64, 512, 3, 3, 1, 1, 256, 256, 5, ((1, 0), (2, 0))),
])
@pytest.mark.skipif(not is_blackwell(), reason="Requires Blackwell GPU (SM 10.x)")
def test_conv2d_im2col_multicta_ws(N, Ci, H, W, Co, R, S, stride, padding, block_size_n, epilogue_block_n, stages, cga_layout):
    torch.manual_seed(0)
    ...
    triton_out = conv2d_im2col_multicta_ws(
        x_nhwc, w_nhwc, stride=stride, padding=padding,
        block_size_n=block_size_n, epilogue_block_n=epilogue_block_n, stages=stages, cga_layout=cga_layout,
    )
...
for case in BENCH_CASES:
    bench_configs.append(
        triton.testing.Benchmark(
            ...
            line_names=["Gluon (multi-CTA WS)", "PyTorch"],
            styles=[("green", "-"), ("blue", "-")],
            ylabel="TFLOPS",
            plot_name=(
                "Conv2dMultiCTA "
...
def bench(N, H, W, Ci, Co, R, S, stride_val, pad_val, block_size_m, block_size_n, block_size_k, epilogue_block_n,
    ...
    if provider == "gluon":
        fn = lambda: conv2d_im2col_multicta_ws(
            x_nhwc, w_nhwc,
