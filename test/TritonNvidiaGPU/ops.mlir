...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 8 : i32, ttg.target = "cuda:90", "ttg.threads-per-warp" = 32 : i32} {
  ...
  // CHECK: ttng.async_tma_copy_global_to_local {{.*}}[{{.*}}, {{.*}}] input_channels = {{.*}} %{{.*}}, %{{.*}}, {{.*}} : !ttng.tensordesc_im2col<256x128xbf16, {{.*}}conv_filter_shape = [3, 3]{{.*}}>
  ...
  tt.func public @tma_load_im2col_conv_metadata_compact(%desc: !ttng.tensordesc_im2col<256x128xbf16, #nvmma_128, conv_output_shape = [64, 64], conv_filter_shape = [3, 3], element_strides = [1, 1, 1, 1], pixel_box_lower_corner = [-1, -1]>) {
    ...
    %bar = ttg.local_alloc : () -> !ttg.memdesc<1xi64, #shared3, #smem, mutable>
    ttng.init_barrier %bar, 1 : !ttg.memdesc<1xi64, #shared3, #smem, mutable>
    ttng.async_tma_copy_global_to_local %desc[%m, %k] input_channels = %c %buf, %bar, %true : !ttng.tensordesc_im2col<256x128xbf16, #nvmma_128, conv_output_shape = [64, 64], conv_filter_shape = [3, 3], element_strides = [1, 1, 1, 1], pixel_box_lower_corner = [-1, -1]>, !ttg.memdesc<1xi64, #shared3, #smem, mutable> -> !ttg.memdesc<256x128xbf16, #nvmma_128, #smem, mutable>
    tt.return
  }
}
