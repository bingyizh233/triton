...
module attributes {ttg.global_scratch_memory_alignment = 1 : i32, ttg.global_scratch_memory_size = 0 : i32, "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 1 : i32, ttg.shared = 264 : i32, ttg.target = "cuda:100", ttg.tensor_memory_size = 0 : i32, "ttg.threads-per-warp" = 32 : i32, "ttg.total-num-warps" = 1 : i32, "ttng.two-ctas" = false} {
  tt.func public @memcpy_1d_tma_kernel(%in_desc: !tt.tensordesc<tensor<64xf32, #shared>> loc("in_desc"(#loc1)), %in_desc_0: i32 loc("in_desc"(#loc1)), %in_desc_1: i64 loc("in_desc"(#loc1)), %out_desc: !tt.tensordesc<tensor<64xf32, #shared>> loc("out_desc"(#loc1)), %out_desc_2: i32 loc("out_desc"(#loc1)), %out_desc_3: i64 loc("out_desc"(#loc1))) attributes {noinline = false, ttg.global_scratch_memory_alignment = 1 : i32, ttg.global_scratch_memory_size = 0 : i32} {
    %true = arith.constant true loc(#loc)
    %c64_i32 = arith.constant 64 : i32 loc(#loc)
    %c0_i32 = arith.constant 0 : i32 loc(#loc)
    %pid = tt.get_program_id x : i32 loc(#loc16)
    %smem = ttg.local_alloc {allocation.offset = 0 : i32} : () -> !ttg.memdesc<64xf32, #shared, #smem, mutable> loc(#loc17)
    %bar = ttg.local_alloc {allocation.offset = 256 : i32} : () -> !ttg.memdesc<1xi64, #shared1, #smem, mutable> loc(#loc18)
